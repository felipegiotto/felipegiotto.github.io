...
let AppComponent = AppComponent_1 = class AppComponent {
    ...
    montarListaPerguntas() {
        this.perguntas = [];
        for (let ciclo = 1; ciclo <= this.qtdCiclos; ciclo++) {
            for (let numero = this.numeroMinimo; numero <= this.numeroMaximo; numero++) {
                for (let multiplo = 1; multiplo <= AppComponent_1.MULTIPLO_ATE; multiplo++) {
                    const p = new _pergunta__WEBPACK_IMPORTED_MODULE_2__["Pergunta"]();
                    p.numero1 = numero;
                    p.numero2 = multiplo;
    ...
    sortearProximaPergunta() {
        ...
        if (this.perguntas.length > 0) {
            ...
            while (this.opcoesRespostas.length < 6) {
                const opcao = Math.round(Math.random() * this.numeroMaximo * 10);
    ...
    responder(resposta) {
        if (this.perguntaAtual) {
            ...
            const ponto = new _pontuacao_pontuacao__WEBPACK_IMPORTED_MODULE_3__["Pontuacao"]();
            ...
            if (this.perguntaAtual.numero1 * this.perguntaAtual.numero2 === resposta) {
                ...
                if (ponto.tempo <= 2000) {
                    ...
                    const restante = ponto.tempo - 2000;
...
let PontuacaoComponent = class PontuacaoComponent {
    ...
    desenharAlvo() {
        ...
        const raio = this.larguraAlvo / 2;
        const centro = this.larguraCanvas / 2;
    ...
    desenharProximoPonto() {
        this.timeout = undefined;
        if (this.pontuacao.length) {
            const raio = this.larguraAlvo / 2;
            const centro = this.larguraCanvas / 2;
            const ponto = this.pontuacao.shift();
            const tempo = ponto.tempo;
            ...
            const distanciaX = Math.cos(angulo) * distancia;
            const distanciaY = Math.sin(angulo) * distancia;
            // Desenha o tiro
            this.ctx.beginPath();
            ...
            this.ctx.fill();
            this.reproduzirTiro();
            const tempoParaEsperar = Math.min(ponto.tempo / 5, 1000);
            this.timeout = setTimeout(() => this.desenharProximoPonto(), tempoParaEsperar);
        }
    }
    reproduzirTiro() {
        this.tiro.currentTime = 0;
...
let TiroCertoComponent = class TiroCertoComponent {
    constructor() {
...
let TiroErradoComponent = class TiroErradoComponent {
    constructor() {
        this.dismiss = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
